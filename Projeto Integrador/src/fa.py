...
class NewFireflyAlgorithm():
    def __init__(self, D, NP, nFES, alpha, betamin, gamma, LB, UB, function):
        ...
        self.LB = LB.copy()  # lower bound
        self.UB = UB.copy()  # upper bound
        ...
        for i in range(self.NP):
            for j in range(self.D):
                self.Fireflies[i][j] = random.uniform(
                    0, 1) * (self.UB[j] - self.LB[j]) + self.LB[j]
    ...
    def move_ffa(self):
        for i in range(self.NP):
            ...
            for j in range(self.NP):
                ...
                r = math.sqrt(r)
                if self.I[i] > self.I[j]:  # brighter and more attractive, which means, less errors
                    beta0 = 1.0
                    beta = (beta0 - self.betamin) * \
                        math.exp(-self.gamma * math.pow(r, 2.0)) + self.betamin
                    for k in range(self.D):
                        scale = abs(self.UB[k] - self.LB[k])
    ...
    def Run(self):
        ...
        while self.evaluations < self.nFES:
            ...
            self.fbest = self.I[0]
            print("Best Fitness:{}\nBest Param:{}\n\n".format(self.fbest, self.Fireflies[self.Index[0]]))
            # move all fireflies to the better locations
            self.move_ffa() 
            
        
        return self.fbest, self.Fireflies, self.Index